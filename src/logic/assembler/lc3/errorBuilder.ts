...
export default class ErrorBuilder {
  ...
  public operandCount(lineNum: number, tokens: string[]): string {
    ...
    if (tokens[0] == ".blkw") {
      ...
      return this.formatMessage(
        lineNum,
        "Assembler error: attempted to throw operand count error for unknown instruction/directive " +
          tokens[0]
      );
    } else {
      //@ts-ignore
  ...
  public immOperand(lineNum: number, operand: string): string {
    return this.formatMessage(lineNum, "Invalid immediate operand: " + operand);
  }
  ...
  public badMemory(lineNum: number, address: number, value: number): string {
    ...
    return this.formatMessage(
      lineNum,
      "Assembler error: value at address " +
        hexAddr +
        " is too large for one word: " +
        value
    );
  }
  ...
  public nanMemory(lineNum: number, address: number): string {
    ...
    return this.formatMessage(
      lineNum,
      "Assembler error: value at address " + hexAddr + " is NaN"
    );
  }
  ...
  public labelBounds(lineNum: number, label: string, bits: number): string {
    return this.formatMessage(
      lineNum,
      "Program counter offset for label " +
        label +
        " does not fit in " +
        bits +
  ...
  public noLineNumForAddr(addr: number): string {
    ...
    return (
      "Assembler error: no line number stored for memory address " + hexAddr
    );
  }
