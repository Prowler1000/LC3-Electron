...
export default class Parser {
  ...
  public static getImmBitCount(token: string): number | undefined {
  ...
  public parseImmediate(
    ...
    // value does not fit in allowed bits
      ...
      ui.appendConsole(
        this.errorBuilder.immBoundsBits(lineNum, bits, token) + "\n"
      );
      return NaN;
    }
    // parsing successful
    ...
    if (
      ...
      for (let i = 1; i < literal.length - 1; i++) {
        ...
        if (
          literal[i] === "\\" &&
          i < literal.length - 2 &&
          Parser.escapes.has(literal[i + 1])
        ) {
  ...
  public calcLabelOffset(
    ...
    if (labels.has(label)) {
      ...
      if (diff < min || diff > max) {
        ui.appendConsole(
          this.errorBuilder.labelBounds(lineNum, label, bits) + "\n"
        );
        return NaN;
      } else {
        return diff & mask;
  ...
  private asmAluOp(lineNum: number, tokens: string[]): number {
    ...
    if (isNaN(destReg)) return NaN;

    // source register 1
    const source1 = this.parseReg(tokens[2], lineNum);
    if (isNaN(source1)) return NaN;
  ...
  private asmPcLoadStore(
    ...
    if (labels.has(tokens[2])) {
      // @ts-ignore
      const offset = this.calcLabelOffset(
        tokens[2],
        pc,
        labels,
        Parser.immBitCounts.get(tokens[0])!,
  ...
  private asmRegLoadStore(lineNum: number, tokens: string[]): number {
    ...
    if (isNaN(destReg) || isNaN(srcReg) || isNaN(imm)) return NaN;
  ...
  private asmTrap(lineNum: number, code: string): number {
    ...
    if (isNaN(immCode)) return NaN;
    else return 0xf000 | (immCode & 0xff);
  }
  ...
  public parseDirective(
    ...
    switch (tokens[0]) {
      ...
      case ".fill":
        ...
        if (
          tokens[1].match(/^[a-zA-Z_]/) != null &&
          tokens[1].match(/^[xX][0-9a-fA-F]+$/) == null &&
          tokens[1].match(/^[bB][0-1]+$/) == null
        ) {
          inc = 1;
          toFix.set(tokens, pc);
        }
