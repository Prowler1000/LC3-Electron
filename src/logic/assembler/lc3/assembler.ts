...
export default class Assembler {
  ...
  public static async assemble(
    ...
    while (!atEnd && ++lineNum < srcLines.length) {
      ...
      if (currLine) {
        ...
        if (this.directives.has(tokens[0])) {
          ...
          const pcInc = parser.parseDirective(
            lineNum,
            tokens,
            pc,
            memory,
        ...
        // instruction:
          ...
          ui.appendConsole(
            errorBuilder.unknownMnemonic(lineNum, tokens[0]) + "\n"
          );
          hasError = true;
        }
      } // end if
    ...
    for (let i = 0; i < memory.length; i++) {
      ...
      if (memory[i] > 0xffff) {
        ui.appendConsole(
          errorBuilder.badMemory(lastLineNum, i + startOffset, memory[i]) + "\n"
        );
        hasError = true;
        result[i + 1] = 0;
      } else if (isNaN(memory[i])) {
        ui.appendConsole(
          errorBuilder.nanMemory(lastLineNum, i + startOffset) + "\n"
        );
        hasError = true;
        result[i + 1] = 0;
      } else {
   ...
   * @param startOffset
   */
  private static async makeSymbolTableBlob(
